...
const CFG = {
  ...
  // 2026/08/29 に 30→13、09/01 に 13→8、同日 8→13 に戻した。
  ...
  // 大きいエッジは**異常ではない**。プールが小さいほど初期プール金の占める割合が
  // 大きく、未成立組なら1口で実効21倍なので +300% は普通に出る。市場が薄い時期は
  // 自分の予想と周りの予想がそもそも噛み合わない。
...
async function analyseTrifecta(sid, pets, combo, U_, unitsLeft, canBuy) {
  ...
  for (const c of combo) {
    ...
    // 止めない。プールが薄いと素で出る値なので、知らせるだけにする。
    if (edge1 > CFG.WARN_EDGE) {
      log(`R${sid}: エッジ +${(edge1 * 100).toFixed(0)}%（${[c.i, c.j, c.k].map(nameOf).join('→')}）`

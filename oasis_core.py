...
# 2026/08/25 のアプデで 30万 → 20万 に減額（第1〜5レースの賞金増額と引き換え）。
...
# 2026/08/25 のアプデで 40万 → 20万 に減額（賞金増額と引き換え）。
WIN_POOL_SEED       = 200_000
...
# lo / hi は timeline から**直読み**した値（ENGINE_NOTES.md 参照）。
# 消費を変えるパッシブが1つも発動していない区間だけを集めると、消費の最小値・最大値に
# 数千件が**ぴったり**張り付く。それが clamp の両端そのもの。
#   短距離 [2.125, 2.875] 1082/1506件 ・ マイル [2.267, 3.067] 1976/2241件
#   中距離 [2.762, 3.737] 1272/1296件 ・ 長距離 [2.720, 3.680] 1154/ 734件
# 旧値は 中距離lo 2.541 / 長距離lo 2.57 / 短距離hi 2.879 / マイルlo 2.234 とずれていた。
# 直すと 194レースで 1着的中 82.0%→82.5%、残差std 0.0795→0.0785。
STAMINA_COST_LAW = {
    '短距離': {'c': 0.0132, 'lo': 2.125, 'hi': 2.875, 'n_seg': 10},
    'マイル': {'c': 0.0197, 'lo': 2.267, 'hi': 3.067, 'n_seg': 15},
    '中距離': {'c': 0.03065, 'lo': 2.762, 'hi': 3.737, 'n_seg': 20},
    '長距離': {'c': 0.04109, 'lo': 2.720, 'hi': 3.680, 'n_seg': 25},
}
...
# --- σ の固定（2026/09/04）---------------------------------------------
# 自動校正（_calibrate_sigma の最尤値）は 09/01 の再学習で
#   単勝 0.0127 → 0.0215 ／ 3連単 0.0137 → 0.0254 とほぼ倍になった。
# その翌日から自動購入の成績が崩れた（bot のDMログで実測）:
#     〜09/01  57レース中 33レースで的中(58%)  回収率 189%  収支 +8,722,962
#     09/02〜  13レース中  1レースで的中( 8%)  回収率  14%  収支  -857,143
#   以前の的中率が続いたとして 13レースで1本以下になる確率 = 0.0%（＝偶然ではない）
# σ が大きい → 組の確率が平らになる → 1レースの口数が 14口→8口に減り、
# 本命の並びから人気薄の並びへ金が逃げる、という経路。
...
# --- 安牌モード（2026/09/14）--------------------------------------------
# **3連単にだけ**掛かる的中率の下限。単勝は対象外（実測で較正が合っており、
# 実績も出ているので触らない: 予測80%以上の帯は 予測94.5% → 実測93.9%）。
# 実ベット52レースの3連単155件を着順と突き合わせた帯ごとの実測:
#     予測  0〜15%  n=97  予測 7.8% → 実測 2.1%  回収率 52%
#     予測 15〜25%  n=20  予測20.2% → 実測 5.0%  回収率 48%   ← まだ壊れている
#     予測 25〜35%  n=12  予測29.9% → 実測25.0%  回収率149%
#     予測 35〜50%  n=15  予測43.3% → 実測20.0%  回収率118%
#     予測 50%以上  n=11  予測70.6% → 実測63.6%  回収率207%
# 下限ごとの3連単だけの成績（買えるレース数 / 52）:
#     15% → 143%(37)   20% → 157%(31)   **25% → 167%(27)**
#     30% → 166%(25)   35% → 172%(23)   50% → 207%(11)
# 25%を既定にしたのは、35%以上とほぼ同じ回収率で**買えるレースが多い**ため。
# 40%以上の200%超は n=21/11 と薄く、当たり1本で30ポイント動く。
# 既定オフ（オートパイロットのパネルで切り替え）。
SAFE_P_MIN = 0.25

MARKET_FAV_MIN_OD = 1.0
# 1.0（=実質下限なし）にしたのは、下の実測で od1.0〜1.5 の帯も市場が正しかったため。
#     od1.0〜1.5 実測100%（市場の暗黙確率76%の1.32倍）
# 2.0 で切ると35レース中10レースで市場本命枠が空振り（買えない）になる。
...
def train_model(log_path=None, sigma_override=None, train_from=DEFAULT_TRAIN_FROM,
    ...
    # 自動校正の値より、実収支のある固定値を優先する（上のコメント参照）。
    if not sigma_override and RACE_SIGMA_PIN:
        sigma_note = (f'（自動校正 {race_sigma:.4f} → 固定値 {RACE_SIGMA_PIN} を使用。'
                      '09/02 に自動校正値へ切り替えた直後、13レースで的中1本・回収率14%まで'
                      '落ちたため差し戻し。RACE_SIGMA_PIN を None にすると自動校正に戻る）')
        race_sigma = float(RACE_SIGMA_PIN)
    if not sigma_override and TRI_SIGMA_PIN:
        tri_note = (f'（自動校正 {tri_sigma:.4f} → 固定値 {TRI_SIGMA_PIN} を使用。同上）')
        tri_sigma = float(TRI_SIGMA_PIN)
...
DEFAULT_SETTINGS = dict(
    ...
    model_weight=1.0,
    # モデル的中率がこれ未満の**3連単**の組は買わない（単勝には掛からない）。
    # 2026/09/13 に実ベット52レース（投入 9,101,000 rrc）を着順と突き合わせて決めた値。
    # 予測的中率の帯ごとの実測:
    #     80%以上   n= 33  予測94.5% → 実測93.9%   合っている（単勝の本命）
    #     30〜50%   n= 28  予測39.1% → 実測21.4%
    #     15〜30%   n= 34  予測21.7% → 実測11.8%
    #     **5〜15%  n= 93  予測 9.1% → 実測 1.1%   回収率24%**  ← ここだけ壊れている
    # 93件・投入1,347,000 で的中1本。予測どおりなら8.5本で、1本以下は確率0.2%＝偶然ではない。
    # この帯を切ると 全52レース 123%→140%、σ差し戻し後の34レースで 139%→147%。
    # 0.15 でも 0.30 でも改善するので、閾値合わせではなく帯そのものが損。
    # ⚠ 下げるときは同じ検証をやり直すこと。ここは推定ノイズの話ではなく**較正の穴**。
    min_prob=0.20,
)
